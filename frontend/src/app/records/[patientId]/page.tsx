...
export default function PatientRecordsPage() {
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      ...
      try {
        const res = await fetch(\`${process.env.NEXT_PUBLIC_API_URL}/api/records/patient/\${patientId}\`, {
      ...
      <div className="max-w-4xl mx-auto pt-10">
        <div className="flex items-center justify-between mb-10">
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight flex items-center gap-3">
             <ShieldCheck className="w-8 h-8 text-blue-400" /> Patient Records
          </h1>
          <Link href="/dashboard" className="text-sm text-slate-500 hover:text-slate-900 transition-colors">Back to Dashboard</Link>
        </div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-8 rounded-3xl bg-white shadow-sm border border-slate-100 border border-slate-200 backdrop-blur-xl">
           <h3 className="text-xl font-semibold text-slate-900 mb-6">Medical Vault Access</h3>
           {loading ? (
